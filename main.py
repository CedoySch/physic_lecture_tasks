...
'''
Задание к лекции "Введение. Современная физическая картина мира" Вариант 2

Программа для конвертации координат между различными системами: 
декартовыми, полярными, сферическими и цилиндрическими.

Формулы перехода:

1. Декартовы -> Полярные (2D):
   r = sqrt(x^2 + y^2)
   θ = atan2(y, x)

2. Полярные -> Декартовы (2D):
   x = r * cos(θ)
   y = r * sin(θ)

3. Декартовы -> Сферические (3D):
   r = sqrt(x^2 + y^2 + z^2)
   θ = atan2(y, x) 
   φ = acos(z / r) 

4. Сферические -> Декартовы (3D):
   x = r * sin(φ) * cos(θ)
   y = r * sin(φ) * sin(θ)
   z = r * cos(φ)

5. Декартовы -> Цилиндрические (3D):
   r = sqrt(x^2 + y^2)
   θ = atan2(y, x) 
   z = z

6. Цилиндрические -> Декартовы (3D):
   x = r * cos(θ)
   y = r * sin(θ)
   z = z
'''
